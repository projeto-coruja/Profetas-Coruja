...
public class FontesObrasSearchDAO {
	...
	try {
	    String query = "from FontesObras where titulo like" + string + "OR comentario like"
		    + string + "OR referenciacirculacaoobras like" + string + "OR url like"
		    + string + "OR copiasmasnuscritas like" + string + "OR traducoes like" + string
		    + "OR editor like" + string + "order by titulo";
    ...
    public List<IdentifiedEntity> mainSearchAND(String titulo, String comentario,
	    String ref_circ_obra, String url, String copias_manuscritas, String traducoes,
	    SimpleDate dataImpressao, String editor, String grupoMovimento,
	    SimpleDate anoInicio_grupomovimento, SimpleDate anoFim_grupomovimento,
	    String descricao_grupomovimento, String local_grupomovimento,
	    double latitude_grupomovimento, double longitude_grupomovimento, String localimpressao,
	    double latitude_localimpressao, double longitude_localimpressao, String classificacao,
	    List<IdentifiedEntity> palavraChave, List<IdentifiedEntity> obrasCitadas,
	    List<IdentifiedEntity> leitores, List<IdentifiedEntity> personagens,
	    List<IdentifiedEntity> autores) throws FontesObrasNotFoundException, UnreachableDataBaseException, GroupMovementNotFoundException, LocalNotFoundException, ClassificationNotFoundException {
	// vamos supor que já recebemos a classificação já vem pronta, vamos
	// apenas pesquisar seu id
	...
	try {
	    ...
	    resultgrupoMovimento = dao_grupomovimento.findGrupoMovimentoByAll(grupoMovimento,
		    anoInicio_grupomovimento, anoFim_grupomovimento, descricao_grupomovimento,
		    local_grupomovimento, latitude_grupomovimento, longitude_grupomovimento);
	    ...
	    List<IdentifiedEntity> resultlocal_impressao = dao_local_impressao.findLocalByAll(
		    localimpressao, latitude_localimpressao, longitude_localimpressao);
	    ...
	    String query = "from FontesObrasMO where titulo like "
		    + normalize("'%" + titulo + "%'") + " AND comentario like "
		    + normalize("'%" + comentario + "%'") + " AND refverenciasirculacaoobras like "
		    + normalize("'%" + ref_circ_obra + "%'") + " AND url like"
		    + normalize("'%" + url + "%'") + " AND copiasmasnuscritas like "
		    + normalize("'%" + copias_manuscritas + "%'") + " AND traducoes like "
		    + normalize("'%" + traducoes + "%'") + " AND dataimpressao ="
		    + normalize("'" + dataImpressao + "'") + " AND editor like "
		    + normalize("'%" + editor + "%'") + " AND " + grupomovimento_query + " AND "
		    + classificacao_query + " AND " + localimpressao_query
		...
		for (IdentifiedEntity l : resultSet) {
		    // resultTemp=
		    // manager.findEntity("FROM fontesobrasmo_personagemmo WHERE personagens_id ="
		    // + p.getId() + "AND fontesobrasmo_id="+ l.getId());
		    resultTemp = manager
			    .find("FROM FontesObras fontes INNER JOIN fontes.personagens list WHERE FontesObrasMO.id ="
				    + p.getId() + "AND list.id=" + l.getId());
		}
	    }
		...
		for (IdentifiedEntity l : resultSet) {
		    resultTemp = (manager
			    .find("FROM fontesobrasmo_personagemmo WHERE autorescitados_id ="
				    + a.getId() + "AND fontesobrasmo_id=" + l.getId()));
		}
	    }
	    resultSet = resultTemp;
	    resultTemp = null;
		...
		for (IdentifiedEntity l : resultSet) {
		    resultTemp = (manager
			    .find("FROM fontesobrasmo_personagemmo WHERE leitores_id =" + a.getId()
				    + "AND fontesobrasmo_id=" + l.getId()));
		}
	    }
	    resultSet = resultTemp;
	    resultTemp = null;
		...
		for (IdentifiedEntity l : resultSet) {
		    resultTemp = (manager
			    .find("FROM fontesobrasmo_fontesobrasmmo WHERE obrascitadas_id ="
				    + o.getId() + "AND fontesobrasmo_id=" + l.getId()));
		}
	    }
	    resultSet = resultTemp;
	    resultTemp = null;
		...
		for (IdentifiedEntity l : resultSet) {
		    resultTemp = (manager
			    .find("FROM fontesobrasmo_palavrachavemmo WHERE palavrachave_id ="
				    + p.getId() + "AND fontesobrasmo_id=" + l.getId()));
		}
	    }
	    resultSet = resultTemp;
    ...
    public List<IdentifiedEntity> findFontesObrasByAll(String titulo, String comentario,
	    String ref_circ_obra, String url, String copias_manuscritas, String traducoes,
	    SimpleDate dataImpressao, String editor, String grupoMovimento,
	    SimpleDate anoInicio_grupomovimento, SimpleDate anoFim_grupomovimento,
	    String descricao_grupomovimento, String local_grupomovimento,
	    double latitude_grupomovimento, double longitude_grupomovimento, String localimpressao,
	    double latitude_localimpressao, double longitude_localimpressao, String classificacao) throws FontesObrasNotFoundException, UnreachableDataBaseException, GroupMovementNotFoundException, LocalNotFoundException, ClassificationNotFoundException {
	// vamos supor que já recebemos a classificação já vem pronta, vamos
	// apenas pesquisar seu id
	...
	try {
	    ...
	    String query = "from FontesObrasMO where titulo like "
		    + (titulo == null || titulo.equals("") ? "'%_%'" : normalize("'%" + titulo
			    + "%'"))
		    + (comentario == null || comentario.equals("") ? "" : " AND comentario like "
			    + normalize("'%" + comentario + "%'"))
		    + (ref_circ_obra == null || ref_circ_obra.equals(" ") ? ""
			    : " AND refverenciasirCulacaoObra like "
				    + normalize("'%" + ref_circ_obra + "%'"))
		    + (url == null || url.equals(" ") ? "" : " AND url like "
			    + normalize("'%" + url + "%'"))
		    + (copias_manuscritas == null || copias_manuscritas.equals("") ? ""
			    : "AND copiasmasnuscritas like "
				    + normalize("'%" + copias_manuscritas + "%'"))
		    + (traducoes == null || traducoes.equals(" ") ? "" : " AND traducoes like "
			    + normalize("'%" + traducoes + "%'"))
		    + (dataImpressao == null || dataImpressao.equals(" ") ? ""
			    : " AND dataimpressao =" + normalize("'" + dataImpressao + "'"))
		    + (editor == null || editor.equals(" ") ? "" : " AND editor like "
			    + normalize("'%" + editor + "%'"))
		    + (grupomovimento_query == null || grupomovimento_query.equals("(  ) ") ? ""
			    : " AND " + grupomovimento_query)
		    + (classificacao_query == null || classificacao_query.equals("(  ) ") ? ""
			    : " AND " + classificacao_query)
		    + (localimpressao_query == null || localimpressao_query.equals("(  ) ") ? ""
	...
	List<IdentifiedEntity> resultSet = null;
	try {
	    resultSet = manager.find("FROM FontesObras" + " where id = " + id + " "
		    + " ORDER BY titulo");
	...
	List<IdentifiedEntity> resultSet = null;
	try {
	    resultSet = manager.find("FROM FontesObras" + " where titulo = '" + titulo
		    + "' AND editor = '" + editor + "'" + // "o'neil" >
							  // "o''neil"
		    " ORDER BY titulo ");
     ...
     * 
     * @param titulo
     *            - titulo da fonte/obra do personagem.
     * @throws UnreachableDataBaseException
     * @throws FontesObrasNotFoundException
     */
	...
	try {
	    ...
	    if (resultSet == null) {
		throw new FontesObrasNotFoundException(
			"Não existe nenhuma Fontes/Obras cadastrado.");
	    } else {
		return resultSet;
	    }
	} catch (DataAccessLayerException e) {
     ...
     *            -copiasManuscritas da fonte/obra.
     * @throws UnreachableDataBaseException
     * @throws FontesObrasNotFoundException
     */
	...
	List<IdentifiedEntity> resultSet = null;
	try {
	    resultSet = manager.find("from FontesObrasMO where copiasManuscritas like '%"
		    + copiasManuscritas + "%' " + "order by titulo");
     ...
     * 
     * @param dataImpressaos
     *            - dataImpressao da fonte/obra.
     * @throws UnreachableDataBaseException
     * @throws FontesObrasNotFoundException
     */
	...
	List<IdentifiedEntity> resultSet = null;
	try {
	    resultSet = manager.find("from FontesObrasMO where dataImpressao like '%"
		    + dataImpressao + "%' " + "order by titulo");
     ...
     * 
     * @param id
     *            - id da Classificacao da fonte/obra.
     * @throws UnreachableDataBaseException
     * @throws FontesObrasNotFoundException
     */
    public List<FontesObras> findFontesObrasByClassificacao(long id) throws UnreachableDataBaseException, FontesObrasNotFoundException {
	List<FontesObras> resultSet = null;
	try {
