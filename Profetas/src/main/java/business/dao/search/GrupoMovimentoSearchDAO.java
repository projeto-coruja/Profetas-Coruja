...
import static business.dao.search.DAOUtility.normalize;
...
public class GrupoMovimentoSearchDAO {
	...
	List<GrupoMovimento> resultSet = manager.find("FROM GrupoMovimento WHERE nome like "
		+ busca + "' OR descricao like " + busca + " ORDER BY nome");
	if (resultSet == null) {
    ...
    public List<IdentifiedEntity> mainSearchAND(String nome, SimpleDate anoinicio,
	    SimpleDate anofim, String descricao, List<IdentifiedEntity> local) throws GroupMovementNotFoundException, UnreachableDataBaseException, LocalNotFoundException {
	List<IdentifiedEntity> resultSetGrupos = null;
	List<IdentifiedEntity> resultSet = null;

	try {
	    resultSetGrupos = manager.find("FROM GrupoMovimento WHERE nome like "
		    + normalize("'%" + nome + "%'") + "AND anoinicio  = '" + anoinicio
		    + "' AND anofim = '" + anofim + "' AND descricao like "
		    + normalize("'%" + descricao + "%'") + " ORDER BY nome");
	    if (local != null) {
		for (IdentifiedEntity l : local) {
		    if (resultSetGrupos != null) {
			for (IdentifiedEntity g : resultSetGrupos) {
			    resultSet = manager
				    .find("FROM GrupoMovimentoMO_Local WHERE grupomovimentomo_id = "
					    + g.getId() + " AND local_id = " + l.getId() + "");
			}
		    }
		}
	    }
    ...
    public List<IdentifiedEntity> findGrupoMovimentoByAll(String nome, SimpleDate anoinicio,
	    SimpleDate anofim, String descricao, String local_grupomovimento,
	...
	try {
	    List<IdentifiedEntity> resultados = (List<IdentifiedEntity>) dao.findLocalByAll(
	    ...
	    if (resultSet == null) {
		// throw new GroupMovementNotFoundException
		// ("Grupo  Movimento não encontrado.");
	...
	try {
	    ...
	    resultSet = manager.find("FROM GrupoMovimento WHERE nome = "
		    + normalize("'" + nome + "'") + " ORDER BY nome");
	...
	List<IdentifiedEntity> resultSet = null;
	try {
	    resultSet = manager.find("FROM GrupoMovimento" + " where id = " + id + " "
		    + " ORDER BY nome ");
	...
	List<IdentifiedEntity> resultSet = null;
	try {
	    resultSet = manager.find("FROM GrupoMovimento" + " where anoinicio = " + anoInicio
		    + " " + " ORDER BY nome ");
	...
	List<IdentifiedEntity> resultSet = null;
	try {
	    resultSet = manager.find("FROM GrupoMovimento" + " where anoinicio = " + anofim + " "
		    + " ORDER BY nome ");
	...
	try {
	    ...
	    if (resultSet == null) {
		throw new GroupMovementNotFoundException(
			"Grupo Movimento não encontrado por descricao.");
	    } else {
		return resultSet;
	    }
	...
	try {
	    ...
	    if (resultSet == null) {
		throw new GroupMovementNotFoundException(
			"Não existe nenhum GrupoMovimento cadastrado.");
	    } else {
		return resultSet;
	    }
	} catch (DataAccessLayerException e) {
