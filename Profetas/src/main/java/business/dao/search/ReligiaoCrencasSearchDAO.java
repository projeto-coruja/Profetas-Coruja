...
public class ReligiaoCrencasSearchDAO {
	...
	try {
	    resultSet = manager.find("from religiaocrencas where nome like '%" + nome
	...
	List<ReligiaoCrencas> resultSet = null;
	try {
	    resultSet = manager.find("from religiaocrencas where descricao like '%" + descricao
		    + "% order by descricao");
	    if (resultSet.isEmpty()) {
	...
	try {
	    ...
	    if (resultSet.isEmpty()) {
		throw new ReligionNotFoundException(
			"Não existe nenhuma Religiao Crenca cadastrada.");
	    } else {
		return resultSet;
	    }
	} catch (DataAccessLayerException e) {
