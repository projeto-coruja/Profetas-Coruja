...
public class InitServlet extends HttpServlet {
	...
	for (String p : profiles_names) {
		...
		log.error("Profile '" + p
			+ "' repetido no banco de dados, verificar imediatamente.");
	    }
	}
	...
	if (user == null) {
	    ...
	    pa.save(new UserAccount("Admin", (Profile) (pa
		    .find("from Profile where profile = 'admin'").get(0)), "admin@coruja.com",
		    EJBUtility.getHash("null", "MD5"), null, null));
	} else {
	    user = null;
	}
	...
	if (user == null) {
	    ...
	    pa.save(new UserAccount("Usuário Padrão", (Profile) (pa
		    .find("from Profile where profile = 'user'").get(0)), "user@coruja.com",
		    EJBUtility.getHash("null", "MD5"), null, null));
	...
	String logprop_config = config.getInitParameter("log4j.properties-location");
	...
	System.err.println("Erro iniciando o sistema de log do arquivo de configuração");
