...
public class AdminRegisterServlet extends HttpServlet {
	...
	if (!senha.equals(request.getParameter("confsenha"))) {
	    ...
	    AlertsUtility.alertAndRedirectPage(response,
		    "Senha e verificação de senha diferente! Tente novamente.",
		    "/GraoPara/protected/admin/cadUser.jsp");
		...
		if (auth.allowedOperation(Permissions.addNewUserPermission, session, true)) {
		    ...
		    AlertsUtility.alertAndRedirectPage(response, "Usuário adicionado!",
			    "/GraoPara/protected/admin/cadUser.jsp");
		...
		AlertsUtility.alertAndRedirectPage(response,
			"Erro no banco de dados! Contate o suporte e tente novamente mais tarde.",
			"/GraoPara/protected/admin/cadUser.jsp");
		e.printStackTrace();
	    } catch (IncorrectLoginInformationException e) {

		AlertsUtility.alertAndRedirectPage(response, "Email inválido!",
			"/GraoPara/protected/admin/cadUser.jsp");
	    } catch (DuplicateUserException e) {

		AlertsUtility.alertAndRedirectPage(response, "Email já em uso!",
			"/GraoPara/protected/admin/cadUser.jsp");
		...
		AlertsUtility.alertAndRedirectPage(response, "Operação inválido!",
			"public/index.jsp");
	    }
	}
    }
